def add_effectiveness_label(df):
    ...
    def add_effectiveness_label(df):
        ...
        # Kriteria gabungan: minimal 3 indikator aktif + desa punya anggaran
        df['label_efektivitas'] = df.apply(
            lambda row: "Efektif" if row['skor'] >= 3 and row.get('Jumlah_alokasi_anggaran_untuk_mendukung_kegiatan_stunting', 0) > 0 else "Tidak Efektif",
            axis=1
        )
    return df
